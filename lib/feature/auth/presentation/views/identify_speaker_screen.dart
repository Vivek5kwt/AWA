...
class SpeakerScreenState extends State<SpeakerScreen> with TickerProviderStateMixin {
  ...
  final SpeakerService _speakerService = SpeakerService(); // NEW
  ...
  Future<void> _loadSettingsAndFetch() async {
    ...
    // NEW: init service and load from local
    await _speakerService.init();
    await _fetchSpeakersLocal();
  }

  // ====== NEW: Load speakers from local SpeakerService ======
  Future<void> _fetchSpeakersLocal() async {
    setState(() {
      _loading = true;
      _error = null;
      _speakers = [];
    ...
    try {
      final map = await _speakerService.listRegisteredWithCounts();
      final list = <Speaker>[];
  ...
  // ====== OLD (kept, not used): Server fetch remains intact ======
  ...
  Future<void> _goToAddSpeaker() async {
    ...
    if (result == true) {
      // NEW: refresh local list after adding
      await _fetchSpeakersLocal();
    }
  }

  // ===== NEW: Local delete =====
  Future<void> _deleteLocalSpeaker(Speaker s) async {
    try {
      final ok = await _speakerService.deleteByName(s.name);
      if (ok) {
        setState(() {
          _speakers.removeWhere((e) => e.id == s.id);
        });
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Not found locally'),
