...
class _GroupSpeechToTextScreenState extends State<GroupSpeechToTextScreen> with TickerProviderStateMixin {
  ...
  Future<void> _initAssemblyConnection() async {
    try {
      final url =
          'wss://streaming.assemblyai.com/v3/ws?sample_rate=16000&encoding=pcm_s16le&format_turns=true&language_detection=true';
      ...
      _keepAliveTimer = Timer.periodic(const Duration(seconds: 10), (_) {
        _assemblyChannel?.sink.add(jsonEncode({'type': 'Ping'}));
      });
      ...
      _assemblyChannel!.stream.listen((message) async {
        ...
        try {
          ...
          final type = (data['type'] ?? data['message_type'] ?? '').toString();

          if (data['language_code'] != null) {
            _sessionLanguage = data['language_code'].toString();
            print('🌐 language_code: $_sessionLanguage');
          }
          ...
          switch (type) {
            ...
            case 'FinalTranscript':
              ...
              final wordSpeaker =
                  (data['words'] as List?)?.first['speaker']?.toString();
              final spId = data['speaker']?.toString() ?? wordSpeaker;
              String spName;
              Color spColor = _myColor;
              bool isMe = false;
              if (spId == null || spId == 'unknown') {
                spName = _myName;
                isMe = true;
              } else {
                spName = _getSpeakerName(spId);
                spColor = _getSpeakerColor(spId);
              }
              print('✅ FinalTranscript [$start-$end] $spName: $text');
            ...
            case 'TurnDetected':
              ...
              print('🔀 TurnDetected $spName [$start-$end]');
              unawaited(
            ...
            case 'SessionEnded':
              ...
              break;
            case 'Error':
              print('⚠️ Error from AssemblyAI: $message');
              final err = data['error']?.toString() ?? 'Unknown error';
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('AssemblyAI error: $err'),
        ...
        if (_isRecording && _reconnectAttempts < 2) {
          _reconnectAttempts++;
          print('🔄 Reconnecting... ($_reconnectAttempts)');
          _initAssemblyConnection();
        }
      });
  ...
  Future<void> _startListening() async {
    ...
    const frameBytes = 3200; // 100ms of 16kHz mono PCM16
