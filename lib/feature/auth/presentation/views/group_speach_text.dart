...
class _GroupSpeechToTextScreenState extends State<GroupSpeechToTextScreen> with TickerProviderStateMixin {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          for (var speakerEntry in speakers) {
            ...
            _updateLabelLanguage(label, language);
  ...
  Future<void> _updateLabelLanguage(int label, String language) async {
    final prefs = await SharedPreferences.getInstance();
    final email = prefs.getString('email') ?? '';
    final uri = Uri.parse(
        '${ApiConstants.baseUrl}/update_label_language?email=$email&label=$label&language=$language');
    try {
      await http.get(uri);
    } catch (_) {}
  }
